...
function fieldToSchema(field: SchemaField): object {
  ...
  if (field.type === 'array') {
    if (field.itemType === 'object' && field.itemFields && field.itemFields.length > 0) {
...
export function fieldsToUiSchema(fields: SchemaField[]): Record<string, unknown> {
  ...
  for (const field of fields) {
    ...
    if (field.type === 'array') {
      if (field.uiArrayDisplay === 'table') fieldUi['useArrayInlineTable'] = true
      if (field.uiArrayDisplay === 'dialog') fieldUi['useArrayItemDialog'] = true
    }

    let extraOpts: Record<string, unknown> = {}
    try { if (field.uiOptions) extraOpts = JSON.parse(field.uiOptions) } catch { /* ignore */ }
    ...
    if (field.type === 'array' && field.itemType === 'object' && field.itemFields?.length) {
...
export function applyUiSchemaToFields(
  ...
  return fields.map(field => {
    ...
    const updated = { ...field }
    if (typeof fieldUi['ui:widget'] === 'string') updated.uiWidget = fieldUi['ui:widget']
    if (fieldUi['fullWidth'] === true) updated.uiFullWidth = true
    if (fieldUi['useArrayInlineTable'] === true) updated.uiArrayDisplay = 'table'
    if (fieldUi['useArrayItemDialog'] === true) updated.uiArrayDisplay = 'dialog'
    const opts = fieldUi['ui:options'] as Record<string, unknown> | undefined
    if (opts && Object.keys(opts).length) updated.uiOptions = JSON.stringify(opts, null, 2)
    if (field.type === 'object' && field.children?.length) {
    ...
    if (field.type === 'array' && field.itemType === 'object' && field.itemFields?.length) {
      ...
      if (itemsUi) updated.itemFields = applyUiSchemaToFields(field.itemFields, itemsUi)
    }
    return updated
  })
}
...
function resolveType(propSchema: Record<string, unknown>): FieldType {
  ...
  const valid: FieldType[] = ['string', 'number', 'integer', 'boolean', 'array', 'object']
...
function propToField(
  ...
  const field: SchemaField = {
    ...
    ...(propSchema.description ? { description: propSchema.description as string } : {}),
  }

  if (type === 'string') {
    if (propSchema.format !== undefined)   field.format    = propSchema.format as string
    if (propSchema.minLength !== undefined) field.minLength = propSchema.minLength as number
    if (propSchema.maxLength !== undefined) field.maxLength = propSchema.maxLength as number
    if (propSchema.pattern !== undefined)  field.pattern   = propSchema.pattern as string
    if (Array.isArray(propSchema.enum))    field.enum      = propSchema.enum as string[]
  }

  if (type === 'number' || type === 'integer') {
    if (propSchema.minimum !== undefined) field.minimum = propSchema.minimum as number
    if (propSchema.maximum !== undefined) field.maximum = propSchema.maximum as number
  }
  ...
  if (type === 'array') {
    ...
    if (itemType === 'object' && items.properties && typeof items.properties === 'object') {
      ...
      field.itemFields = Object.entries(items.properties as Record<string, unknown>).map(
        ([k, v]) => propToField(k, v as Record<string, unknown>, itemReq),
      )
    }
  }
  ...
  if (type === 'object') {
    ...
    if (propSchema.properties && typeof propSchema.properties === 'object') {
      ...
      field.children = Object.entries(propSchema.properties as Record<string, unknown>).map(
        ([k, v]) => propToField(k, v as Record<string, unknown>, childReq),
      )
    }
  }
