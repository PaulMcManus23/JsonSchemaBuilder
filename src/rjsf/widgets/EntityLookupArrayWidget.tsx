...
import { EntityLookupWidget, EntityLookupWidgetState } from './EntityLookupWidget'
import { MockDataService } from '../services/MockDataService'
import type { EntitySearchConfig } from '../services/IDataService'
import { SchemaContext } from '../utils/SchemaContext'

export class EntityLookupArrayWidget extends React.Component<WidgetProps, EntityLookupWidgetState> {
      ...
      <TagPicker
        ...
        onChange={(items) => {
          ...
          this.setState({ selectedItems: newItems }, () => {
            this.props.onChange(newItems.length ? newItems.map(i => i.key) : null)
          })
        }}
        onRenderItem={(p) => {
  ...
  private loadNames() {
    ...
    let values: string[] = []
    if (this.props.value && Array.isArray(this.props.value)) values = [...this.props.value]
