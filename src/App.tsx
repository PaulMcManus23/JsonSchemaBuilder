...
import { FluentProvider, Text, Badge, Button, ToggleButton } from '@fluentui/react-components'
...
function updateFieldInTree(
  ...
  return fields.map(f => {
    ...
    let updated = f
    if (f.children) updated = { ...updated, children: updateFieldInTree(f.children, id, updater) }
    if (f.itemFields) updated = { ...updated, itemFields: updateFieldInTree(f.itemFields, id, updater) }
    return updated
  })
}
...
function removeFieldFromTree(fields: SchemaField[], id: string): SchemaField[] {
  return fields
    ...
    .map(f => {
      let updated = f
      if (f.children) updated = { ...updated, children: removeFieldFromTree(f.children, id) }
      if (f.itemFields) updated = { ...updated, itemFields: removeFieldFromTree(f.itemFields, id) }
      return updated
    })
}
...
export default function App() {
  ...
  const [widths, setWidths] = useState<[number, number, number]>(
    () => ls<[number, number, number] | null>('jsb_widths', null) ?? [190, 360, 320]
  )
  const resizingRef = useRef<{ divider: number; startX: number; startWidths: [number, number, number] } | null>(null)
  ...
  useEffect(() => { localStorage.setItem('jsb_widths', JSON.stringify(widths)) }, [widths])
  ...
  useEffect(() => {
    const onMove = (e: MouseEvent) => {
      ...
      if (r.divider === 0) {
        ...
        w[1] = Math.max(PANEL_MIN, r.startWidths[1] - (w[0] - r.startWidths[0]))
      } else if (r.divider === 1) {
        w[1] = Math.max(PANEL_MIN, r.startWidths[1] + delta)
        w[2] = Math.max(PANEL_MIN, r.startWidths[2] - (w[1] - r.startWidths[1]))
      } else {
  ...
  const startResize = (divider: number) => (e: React.MouseEvent) => {
    e.preventDefault()
    resizingRef.current = { divider, startX: e.clientX, startWidths: [...widths] as [number, number, number] }
    document.body.style.cursor = 'col-resize'
    document.body.style.userSelect = 'none'
  }

  // ── Schema state ─────────────────────────────────────────
  const [fields, setFields] = useState<SchemaField[]>(() => ls('jsb_fields', []))
  ...
  const [rawUiSchema, setRawUiSchema] = useState<Record<string, unknown> | null>(null)

  const clearRaw = useCallback(() => { setRawSchema(null); setRawUiSchema(null) }, [])

  useEffect(() => { localStorage.setItem('jsb_fields', JSON.stringify(fields)) }, [fields])
  ...
  const handleDragEnd = useCallback((e: DragEndEvent) => {
    ...
    if (aData?.fromPalette) {
      ...
      if (idx !== -1) {
        setFields(prev => { const n = [...prev]; n.splice(idx + 1, 0, child); return n })
        setSelectedId(child.id)
      }
      return
    }
    ...
    if (active.id !== over.id) {
      ...
      if (oi !== -1 && ni !== -1) setFields(prev => arrayMove(prev, oi, ni))
    }
  }, [fields, clearRaw])

  const handleFieldChange = useCallback((updated: SchemaField) => {
  ...
  const handleImport = useCallback((
    ...
    setFields(importedUiSchema ? applyUiSchemaToFields(importedFields, importedUiSchema) : importedFields)
    ...
    >
      <DndContext sensors={sensors} onDragStart={handleDragStart} onDragEnd={handleDragEnd}>
        <div className="app-header">
          <Text size={400} weight="semibold">JSON Schema Builder</Text>
          <Badge appearance="filled" color="brand" size="small">rjsf</Badge>
          <div style={{ marginLeft: 'auto', display: 'flex', gap: 8, alignItems: 'center' }}>
            ...
            <Button appearance="secondary" size="small" onClick={() => setImportOpen(true)}>
              Import Schema
            </Button>
            <Button appearance="secondary" size="small" onClick={() => setMockOpen(true)}>
              Mock Data
            </Button>
          </div>
        </div>
        ...
        <div className="app-body" ref={containerRef}>
          ...
          <Canvas
            ...
            onClear={() => { setFields([]); setSelectedId(null); clearRaw() }}
            width={widths[1]}
          />
